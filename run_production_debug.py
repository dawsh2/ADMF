#!/usr/bin/env python3
"""
Run production backtest with debug logging enabled to trace behavior.
"""
...
def run_production_with_debug():
    """Run production backtest with debug configuration."""
    print(f"\n{'='*60}")
    print(f"Running Production Backtest with Debug Logging")
    print(f"Time: {datetime.now()}")
    print(f"{'='*60}\n")
    
    # Run production with debug config
    cmd = [
        sys.executable, 
        "run_production_backtest_v2.py",
        "-c", "config/config_debug_comparison.yaml"
    ]
    ...
    try:
        ...
        print(f"Error running production: {e}")
        return 1

if __name__ == "__main__":
    exit_code = run_production_with_debug()
    sys.exit(exit_code)