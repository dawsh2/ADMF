...
#         self._raw_config_data: Optional[Dict[str, Any]] = None 
#         self._config_data: Dict[str, Any] = {} 
        
#         logger.info(f"SimpleConfigLoader instance {id(self)} creating for {config_file_path}.")
#         self._load_and_shield_config()
#         logger.info(f"ConfigLoader {id(self)} initialized. Shielded config ready. Loaded from '{config_file_path}'.")

#     def _load_and_shield_config(self):
#         raw_data_from_yaml: Optional[Dict[str, Any]] = None
#         try:
#             with open(self.config_file_path, 'r') as f:
#                 raw_data_from_yaml = yaml.safe_load(f) 
            
#             if raw_data_from_yaml is None: 
...
#             else:
#                 # --- Ensure it's a standard dict before further processing ---
#                 self._raw_config_data = dict(raw_data_from_yaml)
#                 # -------------------------------------------------------------
            
#             self._config_data = copy.deepcopy(self._raw_config_data) 
            
#             logger.info(f"ConfigLoader {id(self)}: Raw configuration data (post-dict conversion, id: {id(self._raw_config_data)}): {self._raw_config_data}") 
